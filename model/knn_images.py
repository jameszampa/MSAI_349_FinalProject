...
from utils.read import read_features_labels,read_data
...
class CustomDataset():
    def __init__(self, dir_path, resize=(60,60)):
...
def dimension_scaling(features):
    ...
    std_devs = np.std(features, axis=1, keepdims=True)

    # Avoid division by zero
    std_devs[std_devs == 0] = 1

    scaled_features = (features - means) / std_devs
...
if __name__ == '__main__':
    ...
    train_features, train_labels = train_dataset.images,train_dataset.labels
    train_features_scaled=dimension_scaling(train_features)
    print(train_features_scaled[0])
    test_features, test_labels = test_dataset.images,test_dataset.labels
    test_features_scaled=dimension_scaling(test_features)
    ...
    # Test
    print("in test")
    predictions_list=classifier.predict(test_features_scaled)
    predictions = [i[0] for i in predictions_list]
    # Evaluate
    evaluate('knn_images', test_labels, predictions)
