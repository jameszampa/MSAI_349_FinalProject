...
class Net(nn.Module):
    def __init__(self, dropout_prob=0.35):
        ...
        self.conv2 = nn.Conv2d(6, 64, 3, padding=1, stride=1)  # out: 64x15x15, after pool: 64x7x7
        self.conv3 = nn.Conv2d(64, 128, 3, padding=1, stride=1)  # Adding another convolutional layer
        self.conv4 = nn.Conv2d(128, 256, 3, padding=1, stride=1)  # Adding one more convolutional layer
        self.conv5 = nn.Conv2d(256, 512, 3, padding=1, stride=1)  # Adding one more convolutional layer
        self.fc1 = nn.Linear(512 * 7 * 7, 120)  # Adjust input size to match your data
    ...
    def forward(self, x):
        ...
        x = F.relu(self.conv3(x))  # Adding another convolutional layer
        x = F.relu(self.conv4(x))  # Adding one more convolutional layer
        x = F.relu(self.conv5(x))  # Adding one more convolutional layer
        x = x.view(-1, 512 * 7 * 7)  # Flatten the tensor for the fully connected layer
        x = F.relu(self.fc1(x))
        x = self.dropout(x)
        x = F.relu(self.fc2(x))
        x = self.dropout(x)
...
def CNN_image_YOLO():
    ...
    full_train_dataset = CustomDataset("/Users/jeongyoon/Desktop/GitBlog/MSAI_349_FinalProject-1/dataset/train_YOLO_new")
    test_dataset = CustomDataset("/Users/jeongyoon/Desktop/GitBlog/MSAI_349_FinalProject-1/dataset/test_YOLO_new")
    ...
    net=fit(net, criterion, optimizer, train_loader, val_loader, num_epochs=70,patience=5)
